...
class QPolicyImage(nn.Module):
    def __init__(self, 
        ...
        super(QPolicyImage, self).__init__()
        ...
        self.qvalue = nn.Sequential(nn.Linear(self.feature_dim + self.dim_state, self.dim_hidden),
                                    activation(),
    ...
    def forward(self, images, states, **kwargs):
        ...
        q_values = self.qvalue(torch.cat([features, states], dim=1))
        return q_values

    def get_action(self, images, states):
        q_values = self.forward(images, states, )
        max_action = q_values.max(dim=1)[1]
        return max_action

class DDPGPolicyImage(nn.Module):
    def __init__(
        self,
        dim_image, 
        feature_dim, 
        ...
        super(DDPGPolicyImage, self).__init__()
    ...
    def forward(self, images, states):
        ...
        action = self.action(torch.cat([features, states], dim=1))
        return action * self.max_action

    def get_action_log_prob(self, images, states):
        action = self.forward(images, states)
        return action, None


class PPOPolicyImage(nn.Module):
    def __init__(
        self,
        dim_image, 
        feature_dim, 
        ...
        super(PPOPolicyImage, self).__init__()
    ...
    def forward(self, images, states):
        ...
        x = self.common(torch.cat([features, states], dim=1))
        mean = self.policy(x)
        if self.use_sac:
    ...
    def get_log_prob(self, images, state, action):
        dist = self.forward(images, state)
        log_prob = dist.log_prob(action)
        return log_prob

    def get_action_log_prob(self, images, states):
        dist = self.forward(images, states)
        action = dist.sample()
        log_prob = dist.log_prob(action)
        return action, log_prob

    def rsample(self, images, states, eps=1e-6):
        dist = self.forward(images, states)
        u = dist.rsample()
    ...
    def get_entropy(self, images, states):
        dist = self.forward(images, states)
...
class DiscretePolicyImage(nn.Module):
    def __init__(
        self,
        dim_image,
        feature_dim, 
        ...
        super(DiscretePolicyImage, self).__init__()
    ...
    def forward(self, states, images):
        ...
        action_probs = self.policy(torch.cat([features, states], dim=1))
        dist = Categorical(action_probs)
        return dist

    def get_log_prob(self, images, states, action):
        dist = self.forward(images, states)
        log_prob = dist.log_prob(action)
        return log_prob
