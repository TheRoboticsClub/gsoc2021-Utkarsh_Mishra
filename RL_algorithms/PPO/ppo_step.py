...
def ppo_step(policy_net, value_net, optimizer_policy, optimizer_value, optim_value_iternum, images, states, actions,
             returns, advantages, old_log_probs, clip_epsilon, l2_reg, ent_coeff=0):
    """update critic"""
    value_loss = None
    for _ in range(optim_value_iternum):
        values_pred = value_net(images, states)
    ...
    """update policy"""
    log_probs = policy_net.get_log_prob(images, states, actions)
    ratio = torch.exp(log_probs - old_log_probs)
    surr1 = ratio * advantages
    surr2 = torch.clamp(ratio, 1.0 - clip_epsilon,
                        1.0 + clip_epsilon) * advantages
    policy_surr = -torch.min(surr1, surr2).mean()
    ent = policy_net.get_entropy(images, states)
